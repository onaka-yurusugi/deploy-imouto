...
- API: `/api/letters`（GitHub mailbox 投函）、`/api/call`（Claude SSE ストリーミング、50 秒 abort）
...
2. **tick の動作確認**（API キーが必要。まだ一度も実行していない）
   ```bash
   ANTHROPIC_API_KEY=... DRY_RUN=1 npx tsx scripts/tick.ts   # commit しない
   ```
...
3. **デプロイナウ初回デプロイ**
   ...
   lolipop env create ANTHROPIC_API_KEY ...
...
- Claude API のモデルは `lib/anthropic.ts` の `MODEL_ID` 一箇所。コストを下げたいときは `effort` を先に触る
